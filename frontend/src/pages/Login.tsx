...
function Login() {
  ...
  function validateForm(){
    ...
    if (formData.password.length < 4) return toast.error("Password must be at least 5 characters");
      ...
      <div className="grid lg:grid-cols-2 min-h-screen">
        <div className="flex flex-col justify-center items-center w-full p-6 sm:p-12">
          ...
          <form className="space-y-6" onSubmit={handleForm}>
            ...
            <button type="submit" className="btn btn-primary w-full mt-6" disabled={isLoggingIn}>{isLoggingIn ? (<Loader2 className="animate-spin"/>):("Sign in")} </button>
          </form>
          <p className="text-sm font-light  py-5 mb-10">Dont have an account ? 
            <Link to={'/signup'}>
